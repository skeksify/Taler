...
var tripController = function(params){
    ...
    this.hide = function(){ $('.trip-block').fadeOut(250); }
}
...
$(document).ready(function(){
    ...
    $(document).keyup(function(e){ if(e.which==32 && !$(e.target).is('input') && !$(e.target).is('textarea')) tc.pp() });
    $(document).keydown(function(e){ if(e.which==32 && !$(e.target).is('input') && !$(e.target).is('textarea')) { console.log(e.preventDefault()) } })
    ...
    $('#shtut').click(function(){ showHidePanels('tutorials', $('#tutorials').is(':visible')); });
    $('#shleg').click(function(){ showHidePanels('legend', $('#legend').is(':visible')); });

    $('#flag_icon_selector').click(function(){
        $('#flag_icon_menu').fadeIn(200);
    });
    $('#flag_icon_menu a').click(function(){
        var new_class = $('i', this).attr('class').split(' ').map(function(_class){ return _class.substr(0,3)=='fa-'?_class:'' }).filter(Boolean)[0];
    ...
    $('#loginps').keyup(function(e){ if(e.which==13) tryLogin() });
    $('#action_box_id .bottom-bar .cancel').click(close_dialog);
    $('#action_box_id form input').keydown(function(e){ if(e.which==13) e.preventDefault(); }); //Disable all form submissions
    $('#konssl .log-bar .text').hover(
        function(){ $(this).addClass('open'); (a = $('#konssl .text')[0]).scrollTop = a.scrollHeight; },
        function(){ $(this).removeClass('open'); (a = $('#konssl .text')[0]).scrollTop = a.scrollHeight; }
    );
});
...
var signUp = function(){
    ...
    if(settings.username && settings.password){
        if(/^[\w\.=-]+@[\w\.-]+\.[\w]{2,3}$/.test(settings.email)){
            ...
            var fail = function(msg){ alert(msg?msg:'Signup Failed, please report to Ben if trying again doesn\'t help (Maybe even a refresh?)') };
...
function mjs(){ return JSON.stringify(chapters); }
function cl(c){
